...
export function ProposalDocument({ proposal, isPublic = false }: ProposalDocumentProps) {
  const { subtotal, taxAmount, grandTotal } = computeTotals(
    proposal.items,
    proposal.taxRate
  );
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="text-right">
            <p className="text-xl font-bold">
              {proposal.user.companyName || proposal.user.name}
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            ...
            <div className="space-y-1">
              ...
              {proposal.user.companyName && (
                <p className="text-slate-600 text-sm">{proposal.user.name}</p>
              )}
              <p className="text-slate-500 text-sm">{proposal.user.email}</p>
              {proposal.user.phone && (
                <p className="text-slate-500 text-sm">{proposal.user.phone}</p>
              )}
              {proposal.user.address && (
                <p className="text-slate-500 text-sm">{proposal.user.address}</p>
              )}
            </div>
          </div>
          <div>
            ...
            <div className="space-y-1">
              ...
              {proposal.clientCompany && (
                <p className="text-slate-600 text-sm">{proposal.clientCompany}</p>
              )}
              <p className="text-slate-500 text-sm">{proposal.clientEmail}</p>
              {proposal.clientPhone && (
                <p className="text-slate-500 text-sm">{proposal.clientPhone}</p>
              )}
            </div>
          </div>
        </div>
